/*
  In this code two osillators are sent through a low pass then a highpass filter
  the dry siganl is mixed in to show how pahse can change the sound
...
AudioSynthWaveform       waveform2;      //xy=95,152
AudioSynthWaveform       waveform1;      //xy=111,84
AudioInputI2S            i2s2;           //xy=117,311
AudioSynthWaveform       waveform3;      //xy=233,394
AudioMixer4              mixer3;         //xy=291,197
AudioAnalyzePeak         peak1;          //xy=387,355
AudioFilterStateVariable filter1;        //xy=435,81
AudioMixer4              mixer1;         //xy=487,234
AudioFilterStateVariable filter2;        //xy=596,70
AudioMixer4              mixer2;         //xy=712,219
...
AudioConnection          patchCord3(i2s2, 0, mixer3, 2);
AudioConnection          patchCord4(waveform3, peak1);
AudioConnection          patchCord5(mixer3, 0, mixer1, 0);
...
AudioControlSGTL5000     sgtl5000_1;     //xy=709,373
...
void setup() {
  ...
  //resonance, aka feedback or q, makes the harmoncis aroudn the cuttof frequency louder
  // over .7 and the resonace will start to come out.
...
void loop() {
  ...
  //filter_freq = (potRead(4) * 10000.0) ; //jsut pot control
  ...
  //the filter has 3 outs, low pass, highpass and band pass
  // if you combine the signals you'll get some interesting new harmonics as some frequencis will be increaded and decreased.
  // inverting the pase of one by having the mixer negaive will cahnge what is phased in our out
  mixer2.gain(0, potRead(6)); //0 to 1 wet signal from filters
  mixer2.gain(1, (potRead(7) * 2.0) - 1.0); //-1.0 to 1.0 dry siganl from oscillators
  ...
  if (peak1.available()) {//must be done to get the reading from the "audio" side
    lfo = peak1.read() * 10.0; //read returns 0-1.0 cahnge th 10.0 for the LFO to have more depth of control over the frequency above 
  }

  melody_rate = 200; //fix the melody rate as we're out of pots and might also jsut use a static note
  ...
  if (current_time - prev_time[1] > melody_rate) {
    ...
    freq[0] = chromatic[20];  //jsut play a constant note
