...
AudioEffectBitcrusher    bitcrusher1;    //xy=460,75
AudioFilterStateVariable filter1;        //xy=604,291
AudioEffectEnvelope      envelope1;      //xy=657,115
AudioEffectDelay         delay1;         //xy=829,381
AudioEffectBitcrusher    bitcrusher2;    //xy=832,177
AudioMixer4              mixer2;         //xy=834,259
AudioMixer4              mixer3;         //xy=979,119
AudioOutputI2S           i2s1;           //xy=1026,257
...
AudioConnection          patchCord8(envelope1, bitcrusher2);
AudioConnection          patchCord9(envelope1, 0, mixer3, 0);
AudioConnection          patchCord10(delay1, 0, mixer2, 1);
AudioConnection          patchCord11(bitcrusher2, 0, mixer3, 1);
AudioConnection          patchCord12(mixer2, 0, i2s1, 0);
AudioConnection          patchCord13(mixer2, 0, i2s1, 1);
AudioConnection          patchCord14(mixer2, delay1);
AudioConnection          patchCord15(mixer3, 0, mixer2, 0);
AudioControlSGTL5000     sgtl5000_1;     //xy=254.49993896484375,119
...
void loop() {
  ...
  mixer2.gain(0, potRead(3) * 2.0); //input signal
  mixer2.gain(1, potRead(7) * 2.0); //feedback from delay

  mixer3.gain(0, 0); //dry
  mixer3.gain(1, 1.0); //from bitcrusher

  crush_lvl = (potRead(2) * 44000.0);
  bitcrusher2.sampleRate(crush_lvl);
  bitcrusher2.bits(16); //must be set 
