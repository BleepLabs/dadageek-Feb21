...
void loop() {
  ...
  if (buttonState(2) == FELL) {
    ...
    unsigned long start_loc = bankstart[bank_sel] + (samplelen[bank_sel] * potRead(7)); //something acting funny here
    unsigned long new_len = (samplelen[bank_sel] * potRead(6));

    sampler0.sample_play_loc(start_loc, new_len);
    sampler1.sample_play_loc(bankstart[bank_sel], samplelen[bank_sel]);
  }
